        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 scrollbar-none">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100">
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2"><CalendarIcon size={12}/> Fecha</p>
              <p className="text-sm font-black text-slate-900 uppercase">
                {isValid(event.date) ? format(event.date, 'dd MMM yyyy', { locale: safeLocale }) : 'N/A'}
              </p>
            </div>
            <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100">
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2"><Clock size={12}/> Tipo</p>
              <p className="text-sm font-black text-slate-900 uppercase">
                {isWo ? (event.woType || 'Preventivo') : (event.isMajor ? 'Hito Mayor' : 'Rutina')}
              </p>
            </div>
          </div>

          {/* ALCANCE TÉCNICO (Similar al panel de activos) */}
          <div className="space-y-4">
             <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
               <LayoutList size={14} className="text-brand" /> 
          ...
          <div className="pt-6 border-t border-slate-100">
            <button 
              ...
              className="w-full h-14 bg-slate-900 text-white rounded-[20px] font-black uppercase tracking-[0.2em] text-[11px] shadow-2xl shadow-slate-200 hover:bg-brand transition-all hover:scale-[1.02] active:scale-95"
            >
...
export default function PmCalendarView() {
  ...
  // Use primitives as memo deps so a new Date() reference each render doesn't
  // invalidate the expensive projection calculation unnecessarily.
    ...
    try {
      ...
      (assetPlans || []).forEach((ap: any) => {
        ...
        (projections || []).forEach(proj => {
          ...
          if (!hasWo) {
            evs.push({
              id: `${ap.id}-${proj.cycleIndex}`,
              type: 'projection',
              date: proj.date,
              title: `${basePlan.name || 'PM'} - ${proj.label}`,
              assetName: asset?.name || 'Activo',
              assetCode: asset?.code || 'S/T',
              tasksNames: proj.tasksNames,
              isMajor: proj.isMajor,
              cycleIndex: proj.cycleIndex
            });
          }
        });
      });
    ...
    <div className="flex h-full bg-slate-50 overflow-hidden relative">
      <div className="flex-1 flex flex-col min-w-0 bg-white">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-white shrink-0">
          <div className="flex items-center gap-4">
            <h2 className="text-xl font-black text-slate-900 capitalize tracking-tighter">
              {format(currentDate, 'MMMM yyyy', { locale: safeLocale })}
            </h2>
            <div className="flex items-center bg-slate-100 p-1 rounded-xl">
              <button onClick={() => setCurrentDate(subMonths(currentDate, 1))} className="p-1.5 hover:bg-white rounded-lg transition-all text-slate-400 hover:text-brand">
                <ChevronLeft size={16} />
              </button>
              ...
              <button onClick={() => setCurrentDate(addMonths(currentDate, 1))} className="p-1.5 hover:bg-white rounded-lg transition-all text-slate-400 hover:text-brand">
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Ingeniería</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-2.5 h-2.5 rounded-full bg-slate-900 shadow-sm" />
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Ejecución</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-7 bg-slate-50 border-b border-slate-100 shrink-0">
          {['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo'].map(d => (
            <div key={d} className="py-2 text-center text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">{d}</div>
          ))}
        </div>

        <div className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-none bg-slate-50/20">
            ...
            {calendarDays.map((day) => {
              ...
              return (
                <div key={day.toString()} className={cn(
                  "min-h-[110px] p-2 border-b border-r border-slate-100 transition-all",
                  !isCurrMonth ? "bg-slate-50/30 opacity-30" : "bg-white",
                  isTodayDay && "bg-brand/[0.02]"
                )}>
                  <span className={cn(
                    "text-[11px] font-black w-6 h-6 flex items-center justify-center rounded-lg mb-2",
                    isTodayDay ? "bg-slate-900 text-white shadow-lg" : "text-slate-400"
                  )}>
                    {format(day, 'd')}
                  </span>
                  <div className="space-y-1">
                    {dayEvents.slice(0, 3).map(e => (
                      <button 
                        ...
                        className={cn(
                          "w-full text-left px-2 py-1.5 rounded-lg text-[9px] font-black truncate transition-all shadow-sm",
                          e.type === 'projection' 
                            ? "bg-indigo-500 text-white shadow-indigo-100 hover:bg-indigo-600" 
      ...
      <aside className="w-80 border-l border-slate-200 bg-white flex flex-col shrink-0">
        <div className="p-6 border-b border-slate-100 bg-slate-50/50">
          <h3 className="font-display font-black text-slate-900 text-sm uppercase tracking-widest">Dashboard Mensual</h3>
        </div>

        <div className="flex-1 overflow-y-auto p-5 space-y-8 scrollbar-none">
          <section>
            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
              <ClipboardList size={12} className="text-brand" /> Pendientes ({pendingWOs.length})
            </h4>
            <div className="space-y-2">
              {pendingWOs.slice(0, 5).map(wo => (
                <div key={wo.id} className="p-3 bg-white border border-slate-100 rounded-xl shadow-sm hover:border-brand/20 cursor-pointer transition-all" onClick={() => setSelectedEvent(wo)}>
                  <p className="text-[10px] font-black text-slate-900 truncate">{wo.title}</p>
                  <p className="text-[8px] font-mono font-bold text-slate-400 mt-1">[{wo.assetCode}] {wo.assetName}</p>
                </div>
              ))}
            </div>
          </section>
          ...
          <section>
            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
              <History size={12} className="text-emerald-500" /> Completados ({completedWOs.length})
            </h4>
            <div className="space-y-2">
              {completedWOs.slice(0, 3).map(wo => (
                <div key={wo.id} className="p-3 bg-emerald-50/50 border border-emerald-100 rounded-xl shadow-sm">
                  <p className="text-[10px] font-black text-emerald-800 truncate">{wo.title}</p>
                  <p className="text-[8px] font-bold text-emerald-600/70 mt-1 uppercase tracking-widest">Finalizado {format(wo.date, 'dd MMM')}</p>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="pt-4 border-t border-slate-100">
            <div className="bg-slate-900 rounded-[24px] p-5 relative overflow-hidden group shadow-2xl shadow-slate-200">
              ...
              <h4 className="text-[10px] font-black text-brand uppercase tracking-[0.2em] mb-3 flex items-center gap-2">
                <BrainCircuit size={14} /> Ingeniería ({projectionEvents.length})
              </h4>
              <div className="space-y-2 relative z-10">
                {projectionEvents.slice(0, 4).map(proj => (
                  <div key={proj.id} className="p-2.5 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-all cursor-pointer" onClick={() => setSelectedEvent(proj)}>
                    <p className="text-[9px] font-black text-white/80 truncate">{proj.title}</p>
                    <p className="text-[8px] font-bold text-brand/60 mt-1 uppercase tracking-tighter">Sugerido {format(proj.date, 'dd MMM')}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/50">
           <button onClick={() => setModule('workorders')} className="w-full py-3 bg-slate-900 text-white rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-brand transition-all">Listado Maestro</button>
        </div>
      </aside>
