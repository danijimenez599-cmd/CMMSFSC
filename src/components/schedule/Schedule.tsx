...
import {
  format, startOfWeek, addDays, addMonths, startOfMonth, getDaysInMonth,
...
function getEventClass(dueDate: string, status?: string, toleranceDays?: number): string {
  ...
  if (status === 'CANCELLED') return 'ev-plan'  // canceladas: gris neutro, no rojo
  const d    = new Date(dueDate)
  const tol  = toleranceDays ?? 3

  if (isPast(d) && !isToday(d)) {
    // Ya pasó la fecha — dentro de tolerancia (naranja) o excedida (rojo)
    const daysLate = differenceInDays(new Date(), d)
    return daysLate <= tol ? 'ev-soon' : 'ev-over'
  }

  // Aún no vence — usar toleranceDays del plan como ventana "próximo"
  const daysUntil = differenceInDays(d, new Date())
  if (daysUntil <= tol) return 'ev-soon'
...
export function Schedule() {
  ...
  const days: Date[] = (() => {
    if (mode === 'week') {
      ...
      // Usar addMonths para navegar meses reales (no offset*30 días)
      const base = startOfMonth(addMonths(today, offset))
  ...
  // 1. Planes activos, filtrando activos no operacionales (no tiene sentido
  //    mostrar PMs de equipos en reparación o fuera de servicio en el calendario)
  ...
  function getEventsForCell(row: ScheduleRow, day: Date) {
    ...
    if (row.ap) {
      ...
      const wosForDay = db.workOrders.filter(
        (wo) => wo.pmPlanId === row.ap!.id &&
                wo.dueDate &&
                isoDate(new Date(wo.dueDate)) === ds &&
                wo.status !== 'CANCELLED'   // canceladas: no tienen lugar en el calendario
      )
    ...
    if (row.standaloneWOs) {
      const wos = row.standaloneWOs.filter((wo) => wo.dueDate && isoDate(new Date(wo.dueDate)) === ds && wo.status !== 'CANCELLED')
      return wos.map((wo) => {
         let sts = wo.status
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex gap-4 flex-wrap text-xs">
        {[
          ...
          { cls:'ev-soon', label:'Próximo / en tolerancia' },
