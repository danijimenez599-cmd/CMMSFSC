...
const ProjectionCard = ({ proj }: { proj: any }) => {
  ...
  // All multipliers active this cycle, e.g. [1,2,3] for C6 or [1,2,3,4] for C12
      ...
      <div className={cn(
        ...
        <button onClick={() => setIsExpanded(!isExpanded)} className="flex items-center justify-between p-5 text-left w-full">
          <div className="space-y-2 flex-1 min-w-0">
            {/* Cycle number + all active X multipliers */}
...
export default function AssetDetailPanel({ onEdit }: AssetDetailPanelProps) {
  const {
    ...
    saveAssetPlan = () => {}, unlinkAssetPlan, projectionMonths, meterProjectionCycles = 8,
  } = useStore() as any;
