  ...
  return (
    <div className="relative pl-10 pb-8 last:pb-0 group">
      ...
      <div className={cn(
        ...
        proj.isMajor 
          ? "bg-white border-brand/20 shadow-xl shadow-brand/5 hover:shadow-brand/10" 
          : "bg-white border-slate-100 shadow-sm hover:shadow-md hover:border-slate-200"
      )}>
        {/* Cabecera Interactiva */}
        <button 
          onClick={() => setIsExpanded(!isExpanded)}
          className="flex items-center justify-between p-5 text-left w-full group/btn"
        >
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-3">
              ...
              <Badge variant={proj.isMajor ? 'brand' : 'neutral'} className="text-[8px] px-2 py-0.5 font-black">
                {proj.isMajor ? 'HITO MAYOR' : 'RUTINA'}
              </Badge>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <div className="text-right hidden sm:block">
              <p className="text-[9px] font-black text-slate-400 uppercase">Ciclo {proj.cycleIndex}</p>
              <p className="text-[10px] font-bold text-slate-900">{proj.tasksNames?.length || 0} Tareas</p>
            </div>
            <div className={cn(
              "w-10 h-10 rounded-2xl flex items-center justify-center transition-all",
              isExpanded ? "bg-slate-900 text-white rotate-180" : "bg-slate-50 text-slate-400 group-hover/btn:bg-slate-100"
            )}>
              <ChevronDown size={18} />
            </div>
          </div>
            ...
            >
              <div className="px-5 pb-6 pt-2 border-t border-slate-50 space-y-4">
                <div className="grid gap-2">
                  {(proj.tasksNames || []).map((tn: string, tIdx: number) => (
                    <div key={tIdx} className="flex items-center gap-4 bg-slate-50/50 p-3 rounded-2xl border border-slate-100 group/task">
                      <div className="w-6 h-6 rounded-lg bg-white flex items-center justify-center text-[10px] font-black text-slate-400 shadow-sm border border-slate-100 group-hover/task:text-brand group-hover/task:border-brand/20 transition-colors">
                        {tIdx + 1}
                      </div>
...
export default function AssetSidePanel() {
  ...
  const activePMs = (activePlans || []).map((ap: any) => {
    try {
      ...
      // Llamada segura al motor de estatus
      ...
      console.error('Error en activePMs:', e);
      return null;
    }
  }).filter(Boolean);
  ...
  const handleAssignPlan = async () => {
    ...
    try {
      ...
      showToast({ type: 'success', title: 'Plan vinculado', message: `${plan.name} activado para este equipo.` });
  ...
  return (
    <div className="w-full flex flex-col h-full bg-slate-50 overflow-hidden font-sans">
      ...
      <div className="px-6 py-5 border-b border-slate-200 bg-white shrink-0 flex items-center justify-between">
        <h3 className="font-display font-black text-slate-900 tracking-tight text-sm uppercase flex items-center gap-2">
          <Target size={16} className="text-brand" />
          Inteligencia de Activo
        </h3>
        <Badge variant="neutral" className="text-[8px] font-black uppercase">v2.0 Beta</Badge>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 space-y-10 scrollbar-none">
        ...
        <div className="flex gap-1 p-1.5 bg-slate-200/50 rounded-[20px] border border-slate-200/50 w-full shadow-inner">
          <button
            ...
            className={cn(
              "flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-[14px] transition-all duration-300",
              activeTab === 'info' ? "bg-white text-brand shadow-md" : "text-slate-400 hover:text-slate-600"
            )}
          >
            Estado Actual
          </button>
          <button
            ...
            className={cn(
              "flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-[14px] transition-all duration-300",
              activeTab === 'schedule' ? "bg-white text-brand shadow-md" : "text-slate-400 hover:text-slate-600"
            )}
          >
            Proyección
          </button>
            ...
            <section>
              <div className="flex items-center justify-between mb-5">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                  ...
                  Trabajos en Curso ({openWorkOrders.length})
                </h4>
              </div>
                ...
                <div className="space-y-3">
                  {openWorkOrders.map((wo: any) => (
                    <motion.div
                      ...
                      className="bg-white border border-slate-200 rounded-[24px] p-5 shadow-sm hover:border-brand/30 transition-all cursor-pointer group relative overflow-hidden"
                      onClick={() => setModule('workorders')}
                    >
                      <div className="flex items-center justify-between mb-3">
                        <span className="font-mono text-[10px] font-black text-brand bg-brand/5 px-2.5 py-1 rounded-xl border border-brand/10">#{wo.woNumber}</span>
                        <Badge variant={wo.status as any} className="text-[8px] px-2">{wo.status}</Badge>
                      </div>
                      <p className="text-xs font-black text-slate-900 group-hover:text-brand transition-colors">
                        {wo.title}
                      </p>
                    </motion.div>
                  ))}
            ...
            <section>
              <div className="flex items-center justify-between mb-5">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                  ...
                  Planes de Ingeniería ({activePMs.length})
                </h4>
              </div>
              ...
              {activePMs.length > 0 ? (
                    ...
                    <div key={pm.id} className="bg-white border border-slate-200 rounded-[28px] p-6 shadow-sm relative overflow-hidden group">
                      <div className="flex items-start justify-between gap-4 mb-5">
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-black text-slate-900 truncate uppercase tracking-tight">{pm.planName}</p>
                          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1 flex items-center gap-1.5">
                            <Clock size={10} /> Disparador: {pm.triggerType}
                          </p>
                        </div>
                        <Badge variant={pm.isOverdue ? 'danger' : 'ok'} className="font-black px-3 py-1">
                          {pm.statusLabel}
                        </Badge>
                      </div>
                      ...
                      <div className="flex items-center justify-between p-3 bg-slate-50 rounded-2xl border border-slate-100">
                         <div className="flex items-center gap-2">
                           <p className="text-[9px] font-black text-slate-400 uppercase">Ciclo Actual</p>
                           <span className="font-mono text-xs font-black text-brand bg-white px-2 py-0.5 rounded-lg shadow-sm">{pm.currentCycleIndex}</span>
                         </div>
                         {pm.nextDueDate && (
                           <div className="text-right">
                             <p className="text-[9px] font-black text-slate-400 uppercase">Próximo Hito</p>
                             <p className="text-[10px] font-bold text-slate-900">{formatDate(pm.nextDueDate)}</p>
                           </div>
                         )}
                      </div>
                    </div>
                ...
                <div className="bg-white border-2 border-dashed border-slate-200 rounded-[32px] p-10 text-center mb-6">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Sin estrategia definida</p>
                </div>
              )}
              ...
              <Button
                variant="outline"
                size="sm"
                className="w-full border-slate-300 border-dashed hover:border-brand rounded-2xl h-12 text-[10px] font-black uppercase tracking-[0.2em]"
                icon={<Plus size={16} />}
            ...
            <div className="p-6 bg-slate-900 rounded-[32px] text-white shadow-2xl relative overflow-hidden group">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-[0.3em] text-brand">Análisis Predictivo</p>
                </div>
                <p className="text-xs font-medium text-slate-300 leading-relaxed">
                  Línea de tiempo calculada para los próximos 24 meses basada en ciclos de mantenimiento preventivo.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              {Array.isArray(activePlans) && activePlans.length > 0 ? activePlans.map((ap: any) => {
                try {
                  ...
                  if (!basePlan || !basePlan.id) return null;

                  const planWithTasks = {
                    ...basePlan,
                    tasks: (Array.isArray(pmTasks) ? pmTasks : []).filter((t: any) => t.pmPlanId === basePlan.id)
                  };
                  ...
                  return (
                    <div key={ap.id} className="pt-4">
                      <div className="flex items-center justify-between mb-8 px-2">
                        <h5 className="text-[11px] font-black text-slate-900 uppercase tracking-widest border-l-4 border-brand pl-3">
                          {basePlan.name}
                        </h5>
                        <Badge variant="neutral" className="text-[9px] font-black">{(projections || []).length} HITOS</Badge>
                      </div>
                  ...
                  return <div className="p-4 bg-red-50 text-red-500 text-[10px]">Error en simulación</div>;
                }
              }) : (
                <div className="py-24 text-center">
                  <Calendar size={32} className="mx-auto text-slate-200 mb-4" />
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">No hay proyecciones para mostrar</p>
                </div>
              )}
            </div>
          </div>
      ...
      <Modal
        ...
        title="Configuración Preventiva"
        size="sm"
        footer={
          <div className="flex gap-4 w-full">
            <Button variant="ghost" onClick={() => setShowAssignModal(false)} className="flex-1 rounded-xl">Cancelar</Button>
            <Button variant="primary" onClick={handleAssignPlan} disabled={!selectedPlanId} className="flex-1 rounded-xl bg-slate-900 hover:bg-brand shadow-xl">Vincular Plan</Button>
          </div>
        }
      >
        <div className="space-y-5 py-4">
           <div className="bg-brand/5 border border-brand/10 p-4 rounded-2xl flex items-start gap-3">
             ...
             <p className="text-[10px] font-medium text-slate-600 leading-relaxed">
               Vincular un plan maestro activará el motor de proyecciones y generará la próxima fecha de mantenimiento sugerida.
             </p>
           </div>
           ...
           <FormField label="Plan Maestro">
             <Select value={selectedPlanId} onChange={e => setSelectedPlanId(e.target.value)}>
               ...
               {pmPlans.map((p: any) => <option key={p.id} value={p.id}>{p.name}</option>)}
             </Select>
           </FormField>

           <div className="bg-slate-50 p-5 rounded-[24px] border border-slate-100 space-y-4">
              <FormField label="Fecha de Inicio">
                <input type="date" className="w-full h-12 px-4 text-xs font-bold border border-slate-200 rounded-xl focus:border-brand outline-none" value={customStartDate} onChange={e => setCustomStartDate(e.target.value)} />
              </FormField>
              {isMeterBased && (
                <FormField label="Lectura de Medidor">
                  <input type="number" className="w-full h-12 px-4 text-xs font-bold border border-slate-200 rounded-xl focus:border-brand outline-none" value={customStartMeter} onChange={e => setCustomStartMeter(e.target.value)} />
                </FormField>
              )}
           </div>
        </div>
