...
export const createWoSlice: StateCreator<WoSlice & { currentUser?: any; inventoryItems?: any[]; adjustStock?: any; recalcNextDue?: any; assetPlans?: any[]; measurementPoints?: any[]; showToast?: any }, [], []> = (set, get) => ({
  ...
  fetchWorkOrders: async () => {
    ...
    // Active WOs: always fetch all (no date limit)
    // Closed WOs: only last 90 days + limit 200 to avoid loading entire history
    const cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - 90);
    const cutoffIso = cutoff.toISOString();

    const [activeRes, historicRes, taskRes, commentRes, usageRes, vendorRes] = await Promise.all([
    ...
    if (activeRes.error || historicRes.error) {
      console.error('Error fetching WOs:', activeRes.error || historicRes.error);
      set({ woLoading: false });
      return;
    }

    const woData = [...(activeRes.data || []), ...(historicRes.data || [])];

    const workOrders: WorkOrder[] = woData.map(w => ({
      id: w.id,
      assetId: w.asset_id,
  ...
  completeWo: async (id, payload, meterValue) => {
    ...
    if (wo.assetPlanId && get().recalcNextDue) {
      ...
      // Prioritize the value entered by the technician in the form;
      // fall back to the current state value only if none was provided.
      ...
      // Persist the closing meter reading so the point stays in sync
  ...
  toggleTask: async (woId, taskId) => {
    ...
    // Optimistic update — reflect immediately in UI without a full refetch
    set(state => ({
    ...
    if (error) {
      ...
      set(state => ({
        woTasks: state.woTasks.map(t =>
          t.id === taskId
            ? { ...t, completed: !completed, completedAt: task.completedAt, completedBy: task.completedBy }
            : t
        ),
      }));
      throw error;
